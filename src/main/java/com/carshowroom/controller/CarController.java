...
public class CarController {
	...
	@RequestMapping("/cars")
	public String showCarPage(@RequestParam(name = "page", defaultValue = "1") int page,@RequestParam(name = "size", defaultValue = "5") int pageSize,Model m) {
	...
	public String showAddCarPage(Model m) {
		Admin admin = adminDao.showAdmin();
		m.addAttribute("admin", admin);
    ...
    public String addCar(@ModelAttribute("car") @Valid Car car, BindingResult result, RedirectAttributes ra) {
        if(result.hasErrors()) {
	...
	public String showViewCarPage(@PathVariable int id, Model m) {
	...
	public String showEditCarPage(@PathVariable int id, Model m) {
		Car car = carDao.findById(id);
		Admin admin = adminDao.showAdmin();
		m.addAttribute("admin", admin);
		m.addAttribute("car", car);
	...
	public String updateCar(@ModelAttribute("car") @Valid Car car, BindingResult result, RedirectAttributes ra) {
		if(result.hasErrors()) {
			return "admin/edit-car";
		}
		carDao.update(car);
